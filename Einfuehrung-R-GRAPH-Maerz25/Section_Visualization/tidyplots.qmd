...
### Übung 1: Korrelation `bill_depth_mm` und `bill_depth_mm`
Plotten Sie die `penguins`-Daten so dass Sie die Korrelation `bill_depth_mm` und `bill_depth_mm` innerhalb der Spezies und speziesübergreifend explorieren können.
